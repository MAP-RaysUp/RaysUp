...
def get_fake_poses(image_batch):
    B, _, H, W = image_batch.shape
    device = image_batch.device
    camera_poses = torch.eye(4, device=device).unsqueeze(0).repeat(B, 1, 1)
    fxfycxcy = torch.tensor([float(W), float(H), W / 2.0, H / 2.0], device=device).unsqueeze(0).repeat(B, 1)
...
def trainer(cfg: DictConfig):
    ...
    all_params = []
    all_params.extend(list(raysup.parameters()))
    print("Number of parameters: ", sum(p.numel() for p in all_params))
    optimizer_raysup = instantiate(cfg.optimizer, params=all_params)
    ...
    for epoch in range(epoch_start, cfg.epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            with autocast(device_type="cuda", enabled=cfg.bfloat16, dtype=torch.bfloat16):
                ...
                poses = get_fake_poses(image_batch)
                raysup_hr_feats = run_model(raysup, [image_batch, lr_feats, poses, (h, w)])

                # ============ Loss RaysUp ============ #
                loss = {"raysup_hr": 0.0}
                loss["raysup_hr"] = criterion(raysup_hr_feats, hr_feats)["total"]

            loss_raysup = loss["raysup_hr"]
            ...
            if batch_idx % FREQ == 0:
                ...
                # Log learning rates
                writer.add_scalar(
                    "Learning Rate RaysUp",
                    optimizer_raysup.param_groups[0]["lr"],
                    current_step,
                )
