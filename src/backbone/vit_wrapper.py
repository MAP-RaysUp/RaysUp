...
def get_patch_size_channels(backbone_name):
    if "vit_small" in backbone_name:
        feats = 384
    elif "vit_pe_spatial_small" in backbone_name:
        feats = 384
    elif "vit_pe_spatial_base" in backbone_name:
        feats = 768
    elif "vit_pe_spatial_large" in backbone_name:
        feats = 1024
    elif "vit_pe_spatial_gigantic" in backbone_name:
        feats = 1536
...
class PretrainedViTWrapper(nn.Module):
    ...
    def forward(
        ...
        """
        if self.name in ["vit_base_patch16_siglip_512.v2_webli","vit_large_patch16_siglip_256.v2_webli"]:
