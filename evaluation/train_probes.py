...
def get_resolution_tag(cfg):
    return f"in{cfg.img_size}_out{cfg.target_size}"
...
class UpsamplerEvaluator:
    ...
    def __init__(self, model, backbone, device, cfg, writer, console, pose_estimator=None):
        ...
        self.device = device
        self.upsampler_name = get_upsampler_name(cfg)
        self.use_da3_poses = self.upsampler_name == "raysup"
        self.pose_estimator = pose_estimator
        if self.use_da3_poses and self.pose_estimator is None:
            raise ValueError("upsampler_name='raysup' requires a DA3 pose_estimator to be passed in.")
    ...
    def process_batch(self, image_batch, target, is_training=True):
        ...
        with torch.no_grad():
            ...
            poses = None
            if self.upsampler_name == "raysup":
                poses = estimate_poses(image_batch, self.pose_estimator, self.device)
            pred = run_upsampler(self.model, self.upsampler_name, image_batch, patch_tokens, (H, W), poses=poses)
    ...
    def simple_inference(self, image_batch):
        ...
        with torch.no_grad():
            ...
            if self.upsampler_name == "raysup":
                poses = estimate_poses(image_batch, self.pose_estimator, self.device)
            features = run_upsampler(self.model, self.upsampler_name, image_batch, hr_feats, (H, W), poses=poses)
...
def main(cfg):
    ...
    resolution_tag = get_resolution_tag(cfg)

    # Setup Classifier
    # Either we train one classifier per backbone.
    checkpoint_dir = f"./checkpoints/{task}"
    os.makedirs(checkpoint_dir, exist_ok=True)
    checkpoint_path = f"{checkpoint_dir}/{upsampler_name}_{resolution_tag}.pth"

    # Create persistent consoles instead of creating new ones each time
    terminal_console = Console()  # Terminal output
    if Path(checkpoint_path).exists():
        file_name = f"eval_{upsampler_name}_{task}_{resolution_tag}.log"
    else:
        file_name = f"train_{upsampler_name}_{task}_{resolution_tag}.log"
    ...
    log_print(f"\n[bold cyan]Backbone input size: {cfg.img_size}[/bold cyan]")
    log_print(f"[bold cyan]Probe output size: {cfg.target_size}[/bold cyan]")
    log_print(f"[bold cyan]Upsampler: {upsampler_name}[/bold cyan]")
    log_print(f"[bold cyan]Use DA3 poses: {upsampler_name == 'raysup'}[/bold cyan]")
    ...
    model = load_upsampler(cfg, device, log_print)

    pose_estimator = None
    if upsampler_name == "raysup":
        pose_estimator = load_da3_pose_estimator(cfg, device, log_print)
    # Setup Dataloaders
    ...
    # Setup Evaluator
    evaluator = UpsamplerEvaluator(model, backbone, device, cfg, writer, file_console, pose_estimator=pose_estimator)
    ...
    if Path(checkpoint_path).exists():
        ...
        with progress:
            # Train the task probe while keeping backbone and upsampler frozen.
