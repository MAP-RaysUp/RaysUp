...
from model_loader import get_upsampler_name, load_upsampler, make_fake_poses, run_upsampler
...
def eval_video_tracking_davis(
    ...
    exp_name = f"davis_vidseg_{cfg.eval.ups_factor}_{get_upsampler_name(cfg)}_{cfg.backbone.name}"
    output_subdir = os.path.join(output_dir, exp_name)
    os.makedirs(output_subdir, exist_ok=True)

    video_folder = os.path.join(output_subdir, os.path.basename(video_dir))
...
def extract_feature(
    ...
    poses = make_fake_poses(img_ups) if upsampler_name == "raysup" else None
    return run_upsampler(upsampler, upsampler_name, img_ups, lr_feats, hr_size, poses=poses)
...
def imwrite_visualized(filename, array, color_palette):
    """Save a palette-colored RGB visualization for a segmentation mask."""
    if np.atleast_3d(array).shape[2] != 1:
        raise Exception("Saving visualized PNGs requires 2D array.")

    im = Image.fromarray(array.astype(np.uint8), mode="P")
    im.putpalette(color_palette.ravel())
    im.convert("RGB").save(filename, format="PNG")


def save_groundtruth_visualizations(annotation_dir, output_dir, color_palette):
    """Export all DAVIS ground-truth masks as palette-colored RGB PNGs."""
    os.makedirs(output_dir, exist_ok=True)

    annotation_paths = sorted(glob.glob(os.path.join(annotation_dir, "*.png")))
    for annotation_path in annotation_paths:
        seg = np.array(Image.open(annotation_path), dtype=np.uint8)
        output_path = os.path.join(output_dir, os.path.basename(annotation_path))
        imwrite_visualized(output_path, seg, color_palette)


def save_rgb_frames(frame_dir, output_dir):
    """Export all video frames as RGB PNGs."""
    os.makedirs(output_dir, exist_ok=True)

    frame_paths = sorted(glob.glob(os.path.join(frame_dir, "*.jpg")))
    for frame_path in frame_paths:
        frame = Image.open(frame_path).convert("RGB")
        frame_name = os.path.splitext(os.path.basename(frame_path))[0] + ".png"
...
def run_video_segmentation(cfg, backbone, upsampler, log_print):
    ...
    video_list = open(os.path.join(cfg.dataset_evaluation.dataroot, "ImageSets/2017/val.txt")).readlines()
    ...
    for i, video_name in enumerate(video_list):
        ...
        video_dir = os.path.join(cfg.dataset_evaluation.dataroot, "JPEGImages/480p/", video_name)
        gt_dir = os.path.join(cfg.dataset_evaluation.dataroot, "Annotations/480p/", video_name)
        frame_list = read_frame_list(video_dir)
        seg_path = frame_list[0].replace("JPEGImages", "Annotations").replace("jpg", "png")

        output_dir = HydraConfig.get().runtime.output_dir
        exp_name = f"davis_vidseg_{cfg.eval.ups_factor}_{get_upsampler_name(cfg)}_{cfg.backbone.name}"
        gt_vis_dir = os.path.join(output_dir, exp_name, "groundtruth_vis", video_name)
        rgb_frame_dir = os.path.join(output_dir, exp_name, "rgb_frames", video_name)
        save_groundtruth_visualizations(gt_dir, gt_vis_dir, color_palette)
        save_rgb_frames(video_dir, rgb_frame_dir)
...
def run_davis_evaluation(cfg, log_print):
    ...
    if os.path.exists(csv_name_global_path) and os.path.exists(csv_name_per_sequence_path):
        ...
        dataset_eval = DAVISEvaluation(davis_root=cfg.dataset_evaluation.dataroot, task="semi-supervised", gt_set="val")
...
def main(cfg: DictConfig):
    ...
    upsampler_name = get_upsampler_name(cfg)
    log_print(f"[bold cyan]Backbone: {cfg.backbone.name}[/bold cyan]")
    log_print(f"[bold cyan]Upsampler: {upsampler_name}[/bold cyan]")
    upsampler = load_upsampler(cfg, device, log_print)
